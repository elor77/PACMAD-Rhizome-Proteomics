# Script Title: Transform Trip2022 Abundance Data to Long Format
# 
# Description: Transforms Trip2022 normalized abundance data from wide to long
...
#
# Input: 
#   - 0_Trip2022_clean-data_removed-NA.csv - Filtered Trip2022 dataset
#
# Output:
#   - 0_Trip2022_log-normalized-abundances_long-format.csv - Long-format data
...
df <- read_csv("0_Trip2022_clean-data_removed-NA.csv")

# Step 2: Select protein_id + normalized abundance columns
df_norm <- df %>%
  select(tmt_batch, protein_id, starts_with("Abundances_Normalized_"))

# Step 3: Pivot to long format and drop NAs
df_norm_long <- df_norm %>%
  pivot_longer(
    cols = -c(tmt_batch, protein_id),
    names_to = "raw_name",
...
write_csv(df_norm_long, "0_Trip2022_log2-normalized-abundances_long-format.csv")
