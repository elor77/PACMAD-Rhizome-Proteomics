# Script Title: Visualize EF1 Phylogenetic Tree with Expression Data and Clade Labels
#
# Description: Constructs and visualizes a phylogenetic tree of EF1 proteins across multiple grass species.
#              Protein differential abundance (log2 fold change) is displayed as a heatmap on the tree.
#              Tree tips are optionally colored by species. Significant proteins are marked with asterisks.
#              Selected clades corresponding to orthogroups are labeled using pre-defined node positions.
#
...
#
# Input Files:
#   - ../1_tables/6a_EF1_protein_og_id_desc.txt - Protein ID to orthogroup and description table
#   - ../1_tables/3b_long_log2FC_DAP_rank_protein_id.csv - Differential accumulation data
#   - ../0_raw_data/Proteome_reference_table.txt - Proteome source table for each species
#   - ../RAxML/RAxML_bestTree.33_EF1_proteins.tree - Phylogenetic tree of EF1 proteins
#
# Output Files:
#   - EF1_proteins_for_extraction.txt - Protein IDs and their proteomes for sequence extraction
#   - EF1_protein_name_mapping.txt - Protein ID to short name mapping
#   - EF1_expression_with_descriptions.txt - Expression data merged with protein descriptions
#   - EF1_tree_basic_species.pdf - Basic tree with heatmap and colored tips
#   - EF1_tree_basic_tips_colored_species.pdf - Tree with tips colored by species
#   - EF1_tree_bracketed_species.pdf - Final tree with clade labels and significance markers
#
# Dependencies:
#   - ape
#   - ggtree
...
#
# Notes:
#   - Tripsacum 2019 and 2022 data are merged into a single "Trip" group.
#   - Significant proteins (adjusted p-value < 0.05) are annotated with an asterisk.
#   - The tree is manually rooted using the outgroup Misin04G386000.
#   - Width and height of output figures are customized for optimal figure layout.
...
data <- read_delim("../1_tables/6a_EF1_protein_og_id_desc.txt", 
...
write_tsv(proteins_for_extraction, "EF1_proteins_for_extraction.txt")

protein_mapping <- expression_with_descriptions %>%
  select(protein_id, short_name) %>%
  distinct()
write_tsv(protein_mapping, "EF1_protein_name_mapping.txt")

write_tsv(expression_with_descriptions, "EF1_expression_with_descriptions.txt")
...
tre <- read.tree("../RAxML/RAxML_bestTree.33_EF1_proteins.tree")
rooted_tre <- root(tre, outgroup = "Misin04G386000", resolve.root = TRUE)
...
print(plot_1)
ggsave("EF1_tree_basic_species.pdf", plot_1, width = 3, height = 5)

# Create and save heatmap with colored tips
plot_2 <- create_tree_heatmap(subTre, log2fc, color_tips = TRUE)
print(plot_2)
ggsave("EF1_tree_basic_tips_colored_species.pdf", plot_2, width = 10, height = 12)
...
# Save final plot
ggsave("EF1_tree_bracketed_species.pdf", plot_bracketed, width = 12, height = 8)
