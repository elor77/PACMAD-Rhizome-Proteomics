...
# Trip19 and Trip22 are collapsed into a single "Td" lineage before counting:
# an orthogroup scores 1 DAP point for Tripsacum if significant in either year.
dap_counts <- df_logfc %>%
  filter(orthogroup %in% common_ogs) %>%
  filter(species %in% c("Ag", "Mg", "Pv", "Sn", "Trip19", "Trip22", "Td")) %>%
  mutate(
    is_dap  = if_else(DAP != "NS" & !is.na(DAP), 1L, 0L),
    species = if_else(species %in% c("Trip19", "Trip22"), "Td", species)
  ) %>%
  group_by(orthogroup, species) %>%
  summarise(is_dap = max(is_dap), .groups = "drop") %>%   # 1 point max per lineage
  group_by(orthogroup) %>%
...
# Recount DAPs excluding Sn, with Trip19/Trip22 collapsed into single Td lineage
