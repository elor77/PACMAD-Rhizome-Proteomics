# Script Title: Transform Panicum Abundance Data to Long Format
# 
# Description: Transforms Panicum normalized abundance data from wide to long
...
#
# Input: 
#   - 0_Pv_clean-data_removed-NA.csv - Filtered Panicum dataset
#
# Output:
#   - 0_Pv_log-normalized-abundances_long-format.csv - Long-format data
...
#In Panicum dataset we are selecting only Dust devil samples.
...
##In Papnicum dataset we are selecting only Dust devil samples.
...
df <- read_csv("0_Pv_clean-data_removed-NA.csv")

# Step 2: Select protein_id + normalized abundance columns (Pv-DD only)
df_norm <- df %>%
  select(
    protein_id,
    matches("^Abundances_Normalized_Pv-DD-")
  )
...
write_csv(df_norm_long, "0_Pv-DD_log-normalized-abundances_long-format.csv")
