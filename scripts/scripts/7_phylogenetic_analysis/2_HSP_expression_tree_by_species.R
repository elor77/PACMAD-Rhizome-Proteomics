# Script Title: Visualize HSP Phylogenetic Tree with Species-Colored Tips and Expression Data
# 
# Description: Constructs and visualizes a phylogenetic tree of heat shock proteins (HSPs) 
#              across multiple grass species. Tip points are colored by species rather than 
#              orthogroup. Differential protein abundance data (log2 fold change) is overlaid 
#              as a heatmap. Significant changes are marked with asterisks, and orthogroup 
#              clades are labeled based on predefined node positions.
#
...
#   - Fig5a_HSP_tree_basic_species.pdf - Basic tree with expression heatmap, tips colored by species
#   - Fig5a_HSP_tree_basic_tips_colored_species.pdf - Alternate tree with colored tips
#   - Fig5a_HSP_tree_p_nodes.pdf - Tree with internal node IDs for reference
#   - Fig5a_HSP_tree_bracketed_species.pdf - Final tree with clade brackets and significant tip markers
#
...
#   - Tips are colored by species instead of orthogroup.
#   - Significant proteins (padj < 0.05) are marked with asterisks.
#   - Clades are manually labeled based on specified node IDs.
#   - Rooting and tree layouts are adjusted for visualization clarity.
...
create_tree_heatmap <- function(tree, data, layout = "rectangular",
  ...
  if (color_tips) {
    ...
    p <- p + geom_tippoint(size = 1, color = tip_colors) + geom_tiplab(size = 2)
  } else {
...
print(plot_1)
ggsave("Fig5a_HSP_tree_basic_species.pdf", plot_1, width = 10, height = 12)

# Create and save heatmap with colored tips
plot_2 <- create_tree_heatmap(subTre, log2fc, color_tips = TRUE)
print(plot_2)
ggsave("Fig5a_HSP_tree_basic_tips_colored_species.pdf", plot_2, width = 10, height = 12)
...
# Save final plot
ggsave("Fig5a_HSP_tree_bracketed_species.pdf", plot_bracketed, width = 14, height = 15)
