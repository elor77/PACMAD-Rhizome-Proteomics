# Script Title: Visualize LEA Phylogenetic Tree with Species-Colored Tips and Expression Data
# 
# Description: Constructs and visualizes a phylogenetic tree of late embryogenesis abundant (LEA) proteins 
#              across multiple grass species. Tip points are colored by species instead of orthogroup. 
#              Differential protein abundance data (log2 fold change) is overlaid as a heatmap. 
#              Significant proteins are marked with asterisks, and orthogroup clades are labeled 
#              based on specific node positions.
#
...
#
# Input: 
#   - ../1_tables/6a_LEA_protein_og_id_desc.txt - LEA protein-to-orthogroup and description mapping
...
#   - ../RAxML/RAxML_bestTree.66_lea_proteins.tree - Phylogenetic tree from RAxML
#
# Output:
#   - LEA_proteins_for_extraction.txt - Protein IDs and corresponding proteomes for sequence retrieval
#   - LEA_protein_name_mapping.txt - Mapping of full protein IDs to short names
#   - LEA_expression_with_descriptions.txt - Final expression dataset with descriptions
#   - 5b_LEA_tree_basic_species.pdf - Basic tree with heatmap, tips colored by species
#   - 5b_LEA_tree_basic_tips_colored_species.pdf - Alternate tree with colored tips
#   - 5b_LEA_tree_p_nodes.pdf - Tree with node IDs labeled for bracket reference
#   - 5b_LEA_tree_bracketed_species.pdf - Final bracketed tree with clade labels and significance markers
#
...
#   - Significant proteins (padj < 0.05) are marked with asterisks.
#   - Clades are manually labeled based on predefined node IDs.
#   - Rooting and tip layouts are adjusted for visualization clarity.
...
data <- read_delim("../1_tables/6a_LEA_protein_og_id_desc.txt", 
...
write_tsv(proteins_for_extraction, "LEA_proteins_for_extraction.txt")

protein_mapping <- expression_with_descriptions %>%
  select(protein_id, short_name) %>%
  distinct()
write_tsv(protein_mapping, "LEA_protein_name_mapping.txt")

write_tsv(expression_with_descriptions, "LEA_expression_with_descriptions.txt")
...
tre <- read.tree("../RAxML/RAxML_bestTree.66_lea_proteins.tree")
rooted_tre <- root(tre, outgroup = "Sn00001aa050014", resolve.root = TRUE)
...
create_tree_heatmap <- function(tree, data, layout = "rectangular",
  ...
  if (color_tips) {
    ...
    n_groups <- length(unique(tip_data$species))
    ...
    species_factor <- factor(tip_data$species)
    tip_colors <- colors[as.numeric(species_factor)]
    p <- p + geom_tippoint(size = 2, color = tip_colors) + geom_tiplab(size = 2)
  } else {
    p <- p + geom_tippoint(size = 2) + geom_tiplab(size = 2)
  }
...
plot_1 <- create_tree_heatmap(subTre, log2fc, color_tips = TRUE)
print(plot_1)
ggsave("5b_LEA_tree_basic_species.pdf", plot_1, width = 10, height = 12)

# Create and save heatmap with colored tips
plot_2 <- create_tree_heatmap(subTre, log2fc, color_tips = TRUE)
print(plot_2)
ggsave("5b_LEA_tree_basic_tips_colored_species.pdf", plot_2, width = 10, height = 12)
...
p_nodes <- ggtree(subTre, layout = "rectangular", branch.length = "none", 
  ...
  #scale_color_viridis_d(option = "F", guide = "none")
  #scale_color_brewer(guide = "none", type = "qual", palette = "Dark2")
  scale_color_discrete(guide = "none")

print(p_nodes)

# Define specific nodes for labeling
og_nodes <- c(106, 104, 36, 26, 113, 115, 37, 44, 43, 45, 117, 49, 95, 85, 78, 123, 120, 74, 5, 6)
...
# Save final plot
ggsave("5b_LEA_tree_bracketed_species.pdf", plot_bracketed, width = 12, height = 8)
