# Script Title: Import and Clean Sorghum (Sn) 2022 TMT Proteomics Data
# 
# Description: Imports TMT proteomics data from Excel for Sorghum 2022 samples,
#              cleans column names, maps TMT labels to sample names, standardizes
#              statistical column naming, and reorders columns for analysis.
#              Generates protein detection statistics by species and season,
#              and filters out rows with missing adjusted p-values.
#
# Author: Elad Oren
# Created: 2025-01-01
...
#
# Input: 
#   - CintaRomay10474963_Sn_Set3_Fr_with2peptides_with-adj-p-values.xlsx
#   - Sn_tmt_labels.txt - Sample metadata and TMT label mapping
#
# Output:
#   - 0_Sn_clean-data.csv - Full cleaned dataset
#   - 0_Sn_clean-data_removed-NA.csv - Filtered dataset without missing p-values
#
# Dependencies: readxl, stringr, dplyr, readr
#
# Notes: Uses regular expressions to standardize column naming patterns for
#        abundance ratios, p-values, and grouped abundances. Removes Fall/Autumn
#        samples and columns with "NA" in their names.
...
protein_data <- read_excel("CintaRomay10474963_Sn_Set3_Fr_with2peptides_with-adj-p-values.xlsx", 
                           sheet = "Proteins")
...
# Load your key
key <- read.delim("Sn_tmt_labels.txt", stringsAsFactors = FALSE)

# Get current colnames
all_cols <- colnames(protein_data)

# Match abundance-related columns with F1_<tmt> and season
pattern <- "^(.*)_F13_(\\d+[A-Z]?)_Sample_.*_(Autumn|Fall|Winter|Summer)$"
...
#remove the NA sample columns (this batch was 15 samples)
...
write_csv(protein_data_renamed, "0_Sn_clean-data.csv")
...
write.csv(protein_data_renamed, "0_Sn_clean-data_removed-NA.csv", row.names = FALSE)
